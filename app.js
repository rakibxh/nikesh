...
function footerCreditHTML() {
  return `
    <div class="footer-credit">
      App by <span class="brand-digi">Digi</span><span class="brand-wizr">wizr</span> —
      <a href="https://digiwizr.com/" target="_blank" rel="noopener">
        <span class="brand-digi">digi</span><span class="brand-wizr">wizr.com</span>
      </a>
    </div>`;
}
  ...
  return `
    <div class="app-header">
      <div style="display:flex;flex-direction:column;align-items:center;flex:1;">
        ...
        ${businessName ? `<div class="business-name-tag">${businessName}</div>` : ''}
      </div>
      ${langSwitcherHTML()}
    </div>
    <div class="screen-body">
...
function renderRecentEntries() {
  ...
  if (!entries) return `<div class="loading-wrap"><span class="spinner" style="border-top-color:var(--navy);border-color:rgba(15,28,53,0.3);"></span></div>`;
...
function renderSaleProductList(list) {
  ...
  return list.map((p, i) => `
    <div class="card product-card ${saleState.selected && saleState.selected.name === p.name ? 'selected' : ''}" onclick="selectSaleProduct(${i}, event)">
...
async function renderSessionScreen() {
  ...
  const html = `
    <div class="screen-body">
      ...
      <div class="section-title">${t('cartEmpty') === '' ? '' : ''}</div>
      ...
      <div class="card cart-item">
        ...
        <div class="cart-top">
          <div>
            ...
            <div class="muted" style="font-size:12px;">${item.size || ''} | ${fmtMoney(item.price)}</div>
          </div>
        </div>
        <div class="qty-row">
        ...
        <div class="field" style="margin:6px 0;">
          <label style="font-size:11px;">${t('itemDiscount')}</label>
          <input type="number" value="${item.itemDiscount || 0}" onchange="setCartDiscount(${i}, this.value)">
        </div>
        <div class="right bold" style="color:var(--emerald);">${fmtMoney(lineTotal)}</div>
      </div>`;
  }).join('');
}
...
async function renderEditAccountScreen() {
  document.getElementById('app').innerHTML = `
    <div class="screen-body">
      ...
      <div class="loading-wrap"><span class="spinner" style="border-top-color:var(--navy);border-color:rgba(15,28,53,0.3);"></span></div>
    </div>`;
...
async function loadDailyReportCharts() {
  ...
  if (doughEl) state.chartInstances.dailyDoughnut = new Chart(doughEl, {
    type: 'doughnut', data: { labels: Object.keys(ctx.expensesByCategory||{}), datasets: [{ data: Object.values(ctx.expensesByCategory||{}) }] },
  ...
  if (barEl) {
    const top10 = (ctx.productTotals||[]).slice(0, 10);
    state.chartInstances.dailyBar = new Chart(barEl, {
      type: 'bar', data: { labels: top10.map((p) => p.itemName), datasets: [{ label: t('top10Products'), data: top10.map((p) => p.totalQty), backgroundColor: '#2ECC71' }] },
