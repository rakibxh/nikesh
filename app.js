...
const translations = {
  en: {
    ...
    renewalReminder: "App service will expire in {days} days. Pay next month's subscription to continue.",
    onTrialDashboard: "On trial: {days} days remaining."
  },
  bn: {
    appName: "নিকেশ",
    ...
    renewalReminder: "মেয়াদ বাকি {days} দিন। আপনার সাবসক্রিপশন ফি পরিশোধ করে সেবা চালু রাখুন।",
    onTrialDashboard: "ট্রায়াল চলছে: {days} দিন বাকি"
  }
};
...
async function refreshAccessStatus() {
  ...
  let statusResp = { status: 'Trial', validUntil: '', activatedOn: '', dateCreated: '' };
  ...
  // Prefer the backend's Date Created (source of truth, works across devices);
  // fall back to the locally stored registration date only if the backend has none.
  const createdDate = parseDDMMYYYYToDate(statusResp.dateCreated);
  const daysElapsed = createdDate
    ? Math.floor((todayMid - createdDate) / 86400000)
    : daysElapsedSinceRegistration();

  let accessLevel = 'trial';
  let trialDay = Math.min(Math.max(daysElapsed, 0) + 1, 3);
  let renewalDaysLeft = null;

  if (statusResp.status === 'Active') {
    const validDate = parseDDMMYYYYToDate(statusResp.validUntil);
    if (validDate) {
      if (todayMid > validDate) {
        accessLevel = 'expired';
      } else {
        const daysUntilExpiry = Math.round((validDate - todayMid) / 86400000);
        if (daysUntilExpiry <= 3) {
          accessLevel = 'renewing';
          renewalDaysLeft = Math.max(0, daysUntilExpiry);
        } else {
          accessLevel = 'active';
        }
      }
  ...
  const result = Object.assign({}, statusResp, {
    accessLevel: accessLevel, trialDay: trialDay, daysElapsed: daysElapsed, renewalDaysLeft: renewalDaysLeft
  });
  ...
  return `
    <div class="app-header">
      ...
      <div class="header-center">
        <img class="logo" src="logo-header.png" alt="Niqesh" decoding="async" onerror="this.style.display='none'">
        ${showBusinessName && businessName ? `<div class="business-name-tag">${businessName}</div>` : ''}
      </div>
      <div class="header-side right">${langSwitcherHTML()}</div>
    </div>`;
}
...
function headerHTML(titleKey, opts) {
  opts = opts || {};
  const backFn = opts.noBack ? null : (opts.onBack || goBack);
  const backBtn = backFn ? `<button class="back-btn" onclick="(${backFn.toString()})()">${icon('arrow-left', 'color:#fff;')}</button>` : '';
  const title = opts.customTitle || t(titleKey);
  return `
    <div class="app-header">
      <div class="header-side left">${backBtn}</div>
      <div class="header-center">
        <img class="logo" src="logo-header.png" alt="Niqesh" decoding="async" onerror="this.style.display='none'">
        ${opts.hideTitle ? '' : `<div class="header-subtitle">${title}</div>`}
      </div>
      <div class="header-side right">${opts.showLang !== false ? langSwitcherHTML() : ''}</div>
    </div>`;
}
...
function footerCreditHTML() {
  return `
    <div class="footer-credit">
      <span>App by <a href="https://digiwizr.com/" target="_blank" rel="noopener"><span class="brand-digi">Digi</span><span class="brand-wizr">wizr</span></a></span>
    </div>`;
}
...
function trialRibbonHTML(accessInfo) {
  ...
  if (accessInfo.accessLevel === 'expired') {
    expired = true;
    messageHtml = `<span>${t('trialExpired')}</span>`;
  } else if (accessInfo.accessLevel === 'renewing') {
    const days = accessInfo.renewalDaysLeft != null ? accessInfo.renewalDaysLeft : 0;
    messageHtml = `<span>${t('renewalReminder').replace('{days}', days)}</span>`;
  } else {
    let textKey = 'trialDay1';
    if (accessInfo.trialDay === 2) textKey = 'trialDay2';
    else if (accessInfo.trialDay >= 3) textKey = 'trialDay3';
    messageHtml = `<span>${t(textKey)}</span>`;
  }

  const xBtn = expired ? '' : `<button class="ribbon-x" onclick="dismissRibbon(event)">${icon('x', 'color:#fff;')}</button>`;
  return `
    <div class="trial-ribbon ${expired ? 'expired' : ''}" onclick="${expired ? '' : "subscriptionOrigin='home';navigate('subscription')"}">
      ${icon('crown', 'color:#fff;')}${messageHtml}${xBtn}
    </div>`;
}
...
function renderHome() {
  ...
  const accessPending = !accessInfo;
  const expired = accessInfo && accessInfo.accessLevel === 'expired';
  const locked = accessPending || expired;

  const actionBtn = (labelKey, iconName, route, isLocked) => `
    <button class="btn ${isLocked ? 'disabled' : ''}" onclick="${isLocked ? (accessPending ? '' : `alert('${t('trialExpired')}')`) : `navigate('${route}')`}">
      ${icon(iconName, `color:${isLocked ? '#888' : '#fff'};`)} ${t(labelKey)} ${(isLocked && !accessPending) ? icon('lock', 'color:#888;') : ''}
    </button>`;
  ...
  return `
    ...
    <div class="screen-body">
      ...
      ${actionBtn('recordSale', 'receipt', 'sale', locked)}
      ${actionBtn('startSaleSession', 'shopping-cart', 'session', locked)}
      ${actionBtn('collectionDue', 'banknote', 'collectionDue', locked)}
      ${actionBtn('recordExpense', 'minus-circle', 'expense', locked)}
...
async function renderSessionScreen() {
  const products = await ensureProducts();
  const sortedProducts = products.slice().sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
  const filtered = sessionSearch ? sortedProducts.filter((p) => p.name.toLowerCase().includes(sessionSearch.toLowerCase())) : sortedProducts;
  const cart = getCart();
      ...
      <div class="card cart-item-v2">
        ...
        <div class="cart-v2-row1">
          ...
          <div class="cart-v2-discount">
            <input type="number" title="${t('itemDiscount')}" placeholder="${t('discountPlaceholder')}" value="${item.itemDiscount ? item.itemDiscount : ''}" onchange="setCartDiscount(${i}, this.value)">
          </div>
        </div>
...
function sessionSearchInput(val) {
  sessionSearch = val;
  const products = (state.productCache || []).slice().sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
  const filtered = val ? products.filter((p) => p.name.toLowerCase().includes(val.toLowerCase())) : products;
  document.getElementById('sessionResults').innerHTML = renderSessionResults(filtered);
  afterRender();
}
function addToCart(i) {
  const products = (state.productCache || []).slice().sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
  const filtered = sessionSearch ? products.filter((p) => p.name.toLowerCase().includes(sessionSearch.toLowerCase())) : products;
  const p = filtered[i];
  const cart = getCart();
...
function renderDashboard() {
  ...
  if (accessInfo) {
    if (accessInfo.accessLevel === 'active') {
      ...
      const days = accessInfo.renewalDaysLeft != null ? accessInfo.renewalDaysLeft : 0;
      statusHTML = `<div class="status-card trial">${icon('clock', 'color:var(--amber);')} ${t('renewalReminder').replace('{days}', days)}</div>`;
    } else if (accessInfo.accessLevel === 'trial') {
      const daysRemaining = Math.max(0, 3 - (typeof accessInfo.daysElapsed === 'number' ? accessInfo.daysElapsed : daysElapsedSinceRegistration()));
      statusHTML = `<div class="status-card trial">${icon('clock', 'color:var(--amber);')} ${t('onTrialDashboard').replace('{days}', daysRemaining)}</div>`;
    } else {
  ...
  return `
    ...
    <div class="screen-body">
      ...
      <button class="btn left-align" onclick="subscriptionOrigin='dashboard';navigate('subscription')">${icon('crown', 'color:#fff;')} ${t('subscriptionPayment')}</button>
    </div>
    ${footerCreditHTML()}`;
}
...
async function renderEditAccountScreen() {
  document.getElementById('app').innerHTML = `
    ${headerHTML('editAccountTitle', { onBack: () => navigate('dashboard') })}
    <div class="screen-body">
...
function renderProductsForm(products) {
  ...
  const html = `
    ${headerHTML('manageProducts', { onBack: () => navigate('dashboard') })}
    <div class="screen-body">
...
async function renderDailyReportScreen() {
  document.getElementById('app').innerHTML = `
    ${headerHTML('dailyReportTitle', { onBack: () => navigate('dashboard') })}
    <div class="screen-body">
...
function renderDailyReportBody() {
  ...
  const html = `
    ...
    <div class="screen-body">
      ...
      <div class="chart-wrap"><canvas id="dailyLineChart" height="180"></canvas></div>
      <div class="chart-wrap"><canvas id="dailyDoughnutChart" height="180"></canvas></div>
      <div class="chart-wrap"><canvas id="dailyBarChart" height="220"></canvas></div>
...
------------------------------------------------------------------- */
let monthlyState = { year: new Date().getFullYear(), month: new Date().getMonth() + 1, data: null, loading: false };
function renderMonthlyReportScreen() {
  const monthLabels = langCode() === 'bn' ? MONTHS_BN : MONTHS_EN;
  const html = `
    ${headerHTML('monthlyReportTitle', { onBack: () => navigate('dashboard') })}
    <div class="screen-body">
      <div class="month-picker-card no-print">
        <div class="year-row">
      ...
      <div id="monthlyReportBody">${monthlyState.data ? renderMonthlyReportBody() : (monthlyState.loading ? `<div class="loading-wrap"><span class="spinner" style="border-top-color:var(--navy);border-color:rgba(29,52,79,0.3);"></span></div>` : '')}</div>
    </div>
    ${footerCreditHTML()}`;
  document.getElementById('app').innerHTML = html;
  afterRender();
}
function changeMonthlyYear(delta) { monthlyState.year += delta; monthlyState.data = null; renderMonthlyReportScreen(); }
...
function renderMonthlyReportBody() {
  ...
  return `
    ...
    <div class="chart-wrap"><canvas id="monthlyLineChart" height="180"></canvas></div>
    <div class="chart-wrap"><canvas id="monthlyDoughnutChart" height="180"></canvas></div>
    <div class="chart-wrap"><canvas id="monthlyBarChart" height="220"></canvas></div>
...
let subscriptionOrigin = 'home';
function renderSubscriptionScreen() {
  const businessName = store.get('businessName') || '';
  return `
    ${headerHTML('subscribeTitle', { onBack: () => navigate(subscriptionOrigin) })}
    <div class="screen-body">
...
async function render() {
  ...
  switch (state.route) {
    ...
    case 'monthlyReport':
      renderMonthlyReportScreen();
      if (!monthlyState.data && !monthlyState.loading) selectMonthlyMonth(monthlyState.month);
      break;
